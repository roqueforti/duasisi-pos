...
function getTransaksiList(statusFilter) {
  ...
  let result = dataHeader.map(r => {
    ...
    return {
      ...
      total: total,
      status: (r[9] === "Approved" || r[5] === "Void" || r[5] === "Batal") ? "Void" : (r[5] || "Diterima"),
      estimasi: r[6],
      petugas: r[7] || "Kasir",
      tipe: r[8] || "SelfService",
      voucher: (r[9] && String(r[9]).trim() !== "" && r[9] !== "None" && r[9] !== "Approved" && r[9] !== "PendingApproval" && r[9] !== "Rejected") ? String(r[9]).trim() : "",
      statusVoid: r[9] || "None",
      alasanVoid: r[10] || "",
      subtotal: Number(r[11]) || Number(r[4]) || 0,
      diskon: Number(r[12]) || 0,
      metodeBayar: r[13] || "",
      statusPembayaran: (r[9] === "Approved" || r[5] === "Void" || r[5] === "Batal") ? "Batal / Void" : (r[14] || "Lunas"),
