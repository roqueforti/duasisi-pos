...
export default function RekapView() {
  ...
  const handleApproveVoid = async (noNota: string, isApproved: boolean) => {
    ...
    try {
      ...
      await showAlert('Gagal memproses permohonan void.', 'error');
    }
  };
    ...
    <div className="p-3 md:p-4 space-y-3 sm:space-y-4 w-full text-slate-700">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-3 sm:p-4 flex items-center justify-between gap-2.5 flex-wrap shadow-xs">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => { loadLaporan(); loadAuditLogs(); loadKasShift(); }}
            disabled={loading}
              ...
              {pendingVoidList.map((tx) => (
                <div key={tx.noNota} className="p-4 border border-rose-200 rounded-xl bg-rose-50/40 flex items-center justify-between gap-4 flex-wrap">
                  <div className="space-y-1 text-xs">
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-700 text-sm">{tx.noNota}</span>
                      <span className="text-[10px] bg-rose-100 text-rose-800 font-bold px-2 py-0.5 rounded">Pengajuan Void</span>
                    </div>
                    <p className="text-slate-600">Pelanggan: <span className="font-semibold text-slate-700">{tx.namaPelanggan}</span> ({tx.noHp || '-'})</p>
