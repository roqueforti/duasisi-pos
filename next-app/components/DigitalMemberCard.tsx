...
export default function DigitalMemberCard({
  ...
  useEffect(() => {
    setLocalStamps75(customer.stamps75 ?? 0);
    setLocalStamps45(customer.stamps45 ?? 0);
  }, [customer.stamps75, customer.stamps45]);
  ...
  const handleDownloadPNG = async () => {
    const targetRef = activeCardType === '75' ? cardFrontRef.current : cardBackRef.current;
    if (!targetRef) return;
    setDownloading(true);
    try {
  ...
  const renderCardSvg = (type: '75' | '45', stamps: number, cardRef: React.RefObject<HTMLDivElement | null>) => {
    const clipId = `logoClip_${type}`;

    return (
      <div 
        ref={cardRef}
        className={`dmc-card-face ${type === '75' ? 'dmc-card-front' : 'dmc-card-back'} select-none`}
